...
function activityContent(item) {
  switch (item.type) {
    case 'opportunity_created':
      return { Icon: UserPlus, iconCls: 'text-blue-500 bg-blue-50 dark:bg-blue-900/30 dark:text-blue-300', text: `creó una oportunidad para ${item.customer}` };
    case 'opportunity_stage_changed': {
      const Icon = item.stage_type === 'won' ? Trophy : item.stage_type === 'lost' ? XCircle : ArrowRightCircle;
      const iconCls = item.stage_type === 'won' ? 'text-green-500 bg-green-50 dark:bg-green-900/30 dark:text-green-300' : item.stage_type === 'lost' ? 'text-red-500 bg-red-50 dark:bg-red-900/30 dark:text-red-300' : 'text-orange-500 bg-orange-50 dark:bg-orange-900/30 dark:text-orange-300';
    ...
    <div className="flex items-end gap-1 h-16">
      {trend.map((t, i) => (
        <div key={i} className="flex-1 flex flex-col items-center justify-end h-full gap-0.5 group relative" style={{ maxWidth: `${w}%` }}>
          ...
          <span className="text-[9px] text-gray-300 dark:text-gray-500 whitespace-nowrap">{t.label}</span>
        </div>
      ))}
    </div>
  );
...
function DashboardSkeleton() {
  return (
    <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-5">
      <div className="h-9 w-64 bg-gray-100 dark:bg-white/5 rounded-lg animate-pulse" />
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[1, 2, 3, 4].map(i => <div key={i} className="h-28 bg-gray-100 dark:bg-white/5 rounded-2xl animate-pulse" />)}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {[1, 2, 3, 4].map(i => <div key={i} className="h-40 bg-gray-100 dark:bg-white/5 rounded-xl animate-pulse" />)}
      </div>
    </div>
  );
}
      ...
      <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-5">
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100 tracking-tight">Dashboard CRM</h1>
              <p className="text-sm text-gray-500 dark:text-gray-500">Conversión, motivos de pérdida y clientes a los que llamar hoy</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <select value={days} onChange={e => setDays(Number(e.target.value))}
              className="border border-gray-300 dark:border-white/10 dark:bg-graphite-2 dark:text-gray-100 rounded-lg px-3 py-2 text-sm">
              <option value={7}>Últimos 7 días</option>
              <option value={30}>Últimos 30 días</option>
              <option value={90}>Últimos 90 días</option>
            </select>
            <button onClick={load} className="p-2 border border-gray-200 dark:border-white/10 rounded-lg hover:bg-gray-50 dark:hover:bg-white/5 transition">
              <RefreshCw size={16} className="text-gray-500 dark:text-gray-500" />
            </button>
          </div>
        </div>
        ...
        {data.forecast && data.forecast.open_count > 0 && (
          <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl p-4 sm:p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[11px] text-gray-400 dark:text-gray-500">— {data.forecast.open_count} oportunidades abiertas</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <p className="text-[11px] text-gray-400 dark:text-gray-500 mb-1">Cierre proyectado este mes</p>
                <p className="text-2xl font-bold text-white">{COP(data.forecast.this_month)}</p>
                <p className="text-[11px] text-gray-500 dark:text-gray-500 mt-0.5">Ponderado por probabilidad de cierre</p>
              </div>
              <div>
                <p className="text-[11px] text-gray-400 dark:text-gray-500 mb-1">Pipeline abierto (ponderado)</p>
                <p className="text-xl font-semibold text-gray-100">{COP(data.forecast.total_open_weighted)}</p>
              </div>
              <div>
                <p className="text-[11px] text-gray-400 dark:text-gray-500 mb-1">Pipeline abierto (bruto)</p>
        ...
        {data.trend && data.trend.length > 1 && (
          <div className="bg-white dark:bg-graphite border border-gray-100 dark:border-white/10 rounded-xl shadow-[0_1px_2px_rgba(15,15,15,0.04)] p-4">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-semibold text-sm text-gray-800 dark:text-gray-200">Oportunidades creadas vs. ganadas</h2>
        ...
        {data.campaign_return && (data.campaign_return.meta_ads || data.campaign_return.other_sources.length > 0) && (
          <div className="bg-white dark:bg-graphite border border-gray-100 dark:border-white/10 rounded-xl shadow-[0_1px_2px_rgba(15,15,15,0.04)] p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="font-semibold text-sm text-gray-800 dark:text-gray-200">Retorno de campañas</h2>
            </div>

            {data.campaign_return.meta_ads ? (
              <div className="flex flex-wrap items-center gap-4 sm:gap-8 bg-purple-50/60 border border-purple-100 dark:bg-purple-900/30 dark:border-purple-800/40 rounded-lg p-3 mb-3">
                <div className="flex items-center gap-2">
                  <Megaphone size={16} className="text-purple-500" />
                  <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">Meta Ads</span>
                </div>
                <div>
                  <p className="text-lg font-bold text-gray-900 dark:text-gray-100">{data.campaign_return.meta_ads.created}</p>
                  <p className="text-[11px] text-gray-500 dark:text-gray-500">Leads en el período</p>
                </div>
                <div>
                  <p className="text-lg font-bold text-emerald-600">{data.campaign_return.meta_ads.won}</p>
                  <p className="text-[11px] text-gray-500 dark:text-gray-500">Ya ganados</p>
                </div>
                <div>
                  <p className="text-lg font-bold text-gray-700 dark:text-gray-300">{pct(data.campaign_return.meta_ads.conversion_rate)}</p>
                  <p className="text-[11px] text-gray-500 dark:text-gray-500">Conversión a venta</p>
                </div>
              </div>
            ) : (
              <p className="text-xs text-gray-400 dark:text-gray-500 mb-3">Sin leads de Meta Ads en este período — conecta la integración desde Ajustes si todavía no lo has hecho.</p>
            )}

            {data.campaign_return.other_sources.length > 0 && (
              <div className="space-y-1.5">
                <p className="text-[11px] text-gray-400 dark:text-gray-500 mb-1">Otros canales, para comparar</p>
                {data.campaign_return.other_sources.map(s => (
                  <div key={s.source} className="flex items-center gap-3 text-xs">
                    <span className="w-32 flex-shrink-0 text-gray-600 dark:text-gray-400 truncate">{SOURCE_LABEL[s.source] || s.source}</span>
                    <span className="text-gray-400 dark:text-gray-500">{s.created} leads</span>
                    <span className="text-emerald-600">{s.won} ganados</span>
                    <span className="text-gray-400 dark:text-gray-500 ml-auto">{pct(s.conversion_rate)}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="bg-white dark:bg-graphite border border-gray-100 dark:border-white/10 rounded-xl shadow-[0_1px_2px_rgba(15,15,15,0.04)] p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="font-semibold text-sm text-gray-800 dark:text-gray-200">Motivos de pérdida</h2>
            </div>
            {data.lost_reasons.length === 0 ? (
              <div className="text-center py-8">
                <Inbox size={24} className="mx-auto mb-2 text-gray-200 dark:text-gray-700" />
                <p className="text-sm text-gray-400 dark:text-gray-500">Sin oportunidades perdidas en el período</p>
              </div>
            ) : (
                ...
                {data.lost_reasons
                  ...
                  .map(r => (
                    <div key={r.reason} className="flex items-center gap-3">
                      <span className="text-xs text-gray-600 dark:text-gray-400 w-28 flex-shrink-0">{LOST_REASON_LABEL[r.reason] || r.reason}</span>
                      ...
                      <span className="text-xs font-semibold text-gray-700 dark:text-gray-300 w-6 text-right">{r.count}</span>
                    </div>
                  ))}
              </div>
            )}
          ...
          <div className="bg-white dark:bg-graphite border border-gray-100 dark:border-white/10 rounded-xl shadow-[0_1px_2px_rgba(15,15,15,0.04)] p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="font-semibold text-sm text-gray-800 dark:text-gray-200">Carga de seguimiento</h2>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {[
                { key: 'pendiente', label: 'Pendientes', cls: 'text-accent bg-accent/10' },
                { key: 'vencida', label: 'Vencidas', cls: 'text-red-600 bg-red-50 dark:bg-red-900/30 dark:text-red-300' },
                { key: 'hecha', label: 'Hechas', cls: 'text-emerald-600 bg-emerald-50 dark:bg-emerald-900/30 dark:text-emerald-300' },
                { key: 'cancelada', label: 'Canceladas', cls: 'text-gray-500 bg-gray-50 dark:bg-white/5 dark:text-gray-500' },
              ].map(s => (
          ...
          <div className="bg-white dark:bg-graphite border border-gray-100 dark:border-white/10 rounded-xl shadow-[0_1px_2px_rgba(15,15,15,0.04)] p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="font-semibold text-sm text-gray-800 dark:text-gray-200">Clientes en riesgo</h2>
            </div>
            {data.at_risk_customers.length === 0 ? (
              <div className="text-center py-8">
                <Inbox size={24} className="mx-auto mb-2 text-gray-200 dark:text-gray-700" />
                <p className="text-sm text-gray-400 dark:text-gray-500">Sin clientes en riesgo</p>
              </div>
            ) : (
              <div className="divide-y divide-gray-50 dark:divide-white/10">
                {data.at_risk_customers.map(c => (
                  <div key={c.id} onClick={() => navigate(`/customers/${c.id}`)}
                    className="flex items-center justify-between py-2 cursor-pointer hover:bg-gray-50 dark:hover:bg-white/5 rounded-lg px-1 transition">
                    <span className="text-sm text-gray-700 dark:text-gray-300">{customerName(c)}</span>
                    <span className="text-xs text-gray-400 dark:text-gray-500">Últ. contacto: {fmtDate(c.last_interaction_at)}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="bg-white dark:bg-graphite border border-gray-100 dark:border-white/10 rounded-xl shadow-[0_1px_2px_rgba(15,15,15,0.04)] p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="font-semibold text-sm text-gray-800 dark:text-gray-200">Próxima recompra / mantenimiento</h2>
            </div>
            {data.upcoming_repurchase.length === 0 ? (
              <div className="text-center py-8">
                <Inbox size={24} className="mx-auto mb-2 text-gray-200 dark:text-gray-700" />
                <p className="text-sm text-gray-400 dark:text-gray-500">Sin disparadores de recompra próximos</p>
              </div>
            ) : (
              <div className="divide-y divide-gray-50 dark:divide-white/10">
        ...
        <div className="bg-white dark:bg-graphite border border-gray-100 dark:border-white/10 rounded-xl shadow-[0_1px_2px_rgba(15,15,15,0.04)] p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="font-semibold text-sm text-gray-800 dark:text-gray-200">Actividad del equipo</h2>
          </div>
          {activityLoading ? (
            <div className="space-y-2">
              {[1, 2, 3].map(i => <div key={i} className="h-10 bg-gray-50 dark:bg-white/5 rounded-lg animate-pulse" />)}
            </div>
          ) : !activity || activity.length === 0 ? (
            <div className="text-center py-8">
              <Inbox size={24} className="mx-auto mb-2 text-gray-200 dark:text-gray-700" />
              <p className="text-sm text-gray-400 dark:text-gray-500">Sin actividad reciente del equipo</p>
            </div>
          ) : (
            <div className="divide-y divide-gray-50 dark:divide-white/10">
              {activity.map((item, i) => {
