  ...
  return (
    <Layout>
      <div className="p-4 md:p-6 space-y-6">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-gray-100">Reportes Financieros</h1>
          <p className="text-sm text-gray-500 dark:text-gray-500 mt-1">Basados solo en asientos contabilizados (posted)</p>
        </div>

        <div className="border-b border-gray-200 dark:border-white/10 flex gap-4">
          {TABS.map((t) => (
            <button
              key={t.id}
              onClick={() => { setData(null); setTab(t.id); }}
              className={`pb-3 text-sm font-medium border-b-2 ${tab === t.id ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 dark:text-gray-500 hover:text-gray-700 hover:dark:text-gray-300'}`}
            >
              {t.label}
            </button>
          ))}
        ...
        <div className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 p-4 flex flex-wrap items-center gap-3">
          {tab === 'balance-general' || tab === 'aging' ? (
            <>
              <span className="text-sm text-gray-500 dark:text-gray-500">Corte al:</span>
              <input type="date" value={range.as_of} onChange={(e) => setRange((r) => ({ ...r, as_of: e.target.value }))} className="px-3 py-1.5 border border-gray-300 dark:border-white/10 rounded-lg text-sm" />
            </>
          ) : (
            <>
              <span className="text-sm text-gray-500 dark:text-gray-500">Periodo:</span>
              <input type="date" value={range.from} onChange={(e) => setRange((r) => ({ ...r, from: e.target.value }))} className="px-3 py-1.5 border border-gray-300 dark:border-white/10 rounded-lg text-sm" />
              <span className="text-gray-400 dark:text-gray-500 text-sm">a</span>
              <input type="date" value={range.to} onChange={(e) => setRange((r) => ({ ...r, to: e.target.value }))} className="px-3 py-1.5 border border-gray-300 dark:border-white/10 rounded-lg text-sm" />
            </>
          )}
          {tab === 'aging' && (
            <select value={agingType} onChange={(e) => setAgingType(e.target.value)} className="px-3 py-1.5 border border-gray-300 dark:border-white/10 rounded-lg text-sm">
          ...
          {branches.length > 1 && (
            <select value={branchId} onChange={(e) => setBranchId(e.target.value)} className="px-3 py-1.5 border border-gray-300 dark:border-white/10 rounded-lg text-sm">
          ...
          <div className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 overflow-hidden">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-white/10">
              <thead className="bg-gray-50 dark:bg-graphite-2">
                <tr>{['Código', 'Cuenta', 'Tipo', 'Débito', 'Crédito'].map((h) => <th key={h} className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-500 uppercase">{h}</th>)}</tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-white/10">
                {data.accounts.length === 0 && <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-400 dark:text-gray-500">Sin movimientos contabilizados en el periodo</td></tr>}
                {data.accounts.map((a) => (
                  <tr key={a.id}>
                    <td className="px-4 py-2 text-sm font-mono text-gray-500 dark:text-gray-500">{a.code}</td>
                    <td className="px-4 py-2 text-sm text-gray-900 dark:text-gray-100">{a.name}</td>
                    <td className="px-4 py-2 text-sm text-gray-500 dark:text-gray-500 capitalize">{a.account_type}</td>
              ...
              <tfoot>
                <tr className="font-semibold border-t border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-graphite-2">
        ...
        {!loading && data && tab === 'balance-general' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 p-4">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Activo</h3>
              {data.activo.map((a) => (
                <div key={a.id} className="flex justify-between text-sm py-1">
                  <span className="text-gray-600 dark:text-gray-400 dark:dark:text-gray-500">{a.code} - {a.name}</span>
                  <span className="text-gray-900 dark:text-gray-100">{formatCurrency(a.balance)}</span>
                </div>
              ))}
              <div className="flex justify-between text-sm font-semibold border-t border-gray-200 dark:border-white/10 mt-2 pt-2">
                <span>Total Activo</span><span>{formatCurrency(data.totales.total_activo)}</span>
              </div>
            </div>
            <div className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 p-4">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Pasivo</h3>
              {data.pasivo.map((a) => (
                <div key={a.id} className="flex justify-between text-sm py-1">
                  <span className="text-gray-600 dark:text-gray-400 dark:dark:text-gray-500">{a.code} - {a.name}</span>
                  <span className="text-gray-900 dark:text-gray-100">{formatCurrency(a.balance)}</span>
              ...
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3 mt-5">Patrimonio</h3>
              ...
              <div className="flex justify-between text-sm py-1 text-gray-500 dark:text-gray-500 italic">
        ...
        {!loading && data && tab === 'income-statement' && (
          <div className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 p-5 max-w-2xl">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Ingresos</h3>
            {data.ingresos.map((a) => (
              <div key={a.id} className="flex justify-between text-sm py-1"><span className="text-gray-600 dark:text-gray-400 dark:dark:text-gray-500">{a.code} - {a.name}</span><span>{formatCurrency(a.total)}</span></div>
            ))}
            <div className="flex justify-between text-sm font-semibold border-t border-gray-200 dark:border-white/10 mt-2 pt-2 mb-4"><span>Total Ingresos</span><span>{formatCurrency(data.totales.total_ingresos)}</span></div>

            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Costo de Ventas</h3>
            {data.costos.map((a) => (
              <div key={a.id} className="flex justify-between text-sm py-1"><span className="text-gray-600 dark:text-gray-400 dark:dark:text-gray-500">{a.code} - {a.name}</span><span>{formatCurrency(a.total)}</span></div>
            ))}
            <div className="flex justify-between text-sm font-semibold border-t border-gray-200 dark:border-white/10 mt-2 pt-2 mb-2"><span>Total Costos</span><span>{formatCurrency(data.totales.total_costos)}</span></div>
            ...
            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Gastos Operativos</h3>
            {data.gastos.map((a) => (
              <div key={a.id} className="flex justify-between text-sm py-1"><span className="text-gray-600 dark:text-gray-400 dark:dark:text-gray-500">{a.code} - {a.name}</span><span>{formatCurrency(a.total)}</span></div>
            ))}
            <div className="flex justify-between text-sm font-semibold border-t border-gray-200 dark:border-white/10 mt-2 pt-2 mb-4"><span>Total Gastos</span><span>{formatCurrency(data.totales.total_gastos)}</span></div>

            <div className={`flex justify-between text-base font-bold border-t-2 border-gray-300 dark:border-white/10 pt-3 ${data.totales.utilidad_neta >= 0 ? 'text-green-700' : 'text-red-700'}`}>
        ...
        {!loading && data && tab === 'comparativo' && (
          <div className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 overflow-hidden">
            <div className="px-4 py-2.5 bg-gray-50 dark:bg-graphite-2 border-b border-gray-200 dark:border-white/10 text-xs text-gray-500 dark:text-gray-500">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-white/10">
                ...
                {data.accounts.map((a) => (
                  <tr key={a.id}>
                    ...
                    <td className="px-4 py-2 text-sm text-right text-gray-500 dark:text-gray-500">{formatCurrency(a.prior_balance)}</td>
                    <td className={`px-4 py-2 text-sm text-right font-medium ${a.variance < 0 ? 'text-red-600' : a.variance > 0 ? 'text-green-600' : 'text-gray-500 dark:text-gray-500'}`}>{formatCurrency(a.variance)}</td>
                    <td className="px-4 py-2 text-sm text-right text-gray-500 dark:text-gray-500">{a.variance_pct === null ? '—' : `${a.variance_pct.toFixed(1)}%`}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {!loading && data && tab === 'cashflow-indirecto' && (
          <div className="space-y-4 max-w-2xl">
            {[
              ...
              <div key={section.title} className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 p-4">
                <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{section.title}</h3>
                {(section.extra || []).map(([label, value]) => (
                  <div key={label} className="flex justify-between text-sm py-1"><span className="text-gray-600 dark:text-gray-400 dark:dark:text-gray-500">{label}</span><span>{formatCurrency(value)}</span></div>
                ))}
                {section.changes.length === 0 && <div className="text-sm text-gray-400 dark:text-gray-500 py-1">Sin movimientos</div>}
                {section.changes.map((c) => (
                  <div key={c.id} className="flex justify-between text-sm py-1"><span className="text-gray-600 dark:text-gray-400 dark:dark:text-gray-500">Δ {c.code} - {c.name}</span><span>{formatCurrency(c.cash_impact)}</span></div>
                ))}
                <div className="flex justify-between text-sm font-semibold border-t border-gray-200 dark:border-white/10 mt-2 pt-2"><span>Efectivo neto</span><span>{formatCurrency(section.total)}</span></div>
              </div>
            ))}
            ...
            <div className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 p-4">
              ...
              <div className="flex justify-between text-sm py-1"><span className="text-gray-600 dark:text-gray-400 dark:dark:text-gray-500">Efectivo al inicio</span><span>{formatCurrency(data.cash.opening)}</span></div>
              <div className="flex justify-between text-sm py-1"><span className="text-gray-600 dark:text-gray-400 dark:dark:text-gray-500">Efectivo al final</span><span>{formatCurrency(data.cash.closing)}</span></div>
            </div>
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500">{data.methodology_note}</p>
          </div>
        )}
        ...
        {!loading && data && tab === 'aging' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              {data.buckets.map((b) => (
                <div key={b.key} className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 p-3">
                  <div className="text-xs text-gray-500 dark:text-gray-500">{b.label}</div>
                  <div className="text-sm font-semibold text-gray-900 dark:text-gray-100">{formatCurrency(b.total)}</div>
                </div>
              ))}
            </div>
            <div className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-white/10">
                <thead className="bg-gray-50 dark:bg-graphite-2">
                  <tr>{['Tercero', 'Sin vencer', '1-30', '31-60', '61-90', '+90', 'Total'].map((h) => <th key={h} className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-500 uppercase">{h}</th>)}</tr>
                </thead>
                <tbody className="divide-y divide-gray-100 dark:divide-white/10">
                  {data.third_parties.length === 0 && <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400 dark:text-gray-500">Sin saldos abiertos a la fecha de corte</td></tr>}
                  {data.third_parties.map((tp) => (
                    <tr key={tp.third_party_id}>
                      <td className="px-4 py-2 text-sm text-gray-900 dark:text-gray-100">{tp.name}</td>
        ...
        {!loading && data && tab === 'retenciones' && (
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 p-3"><div className="text-xs text-gray-500 dark:text-gray-500">Base gravable</div><div className="text-lg font-semibold">{formatCurrency(data.totals.base)}</div></div>
              <div className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 p-3"><div className="text-xs text-gray-500 dark:text-gray-500">ReteFuente</div><div className="text-lg font-semibold">{formatCurrency(data.totals.retefuente)}</div></div>
              <div className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 p-3"><div className="text-xs text-gray-500 dark:text-gray-500">ReteICA</div><div className="text-lg font-semibold">{formatCurrency(data.totals.reteica)}</div></div>
            </div>
            {data.customers.length === 0 && (
              <div className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 p-8 text-center text-gray-400 dark:text-gray-500">Ningún cliente practicó retención en el periodo seleccionado</div>
            )}
            {data.customers.map((c) => (
              <div key={c.customer_id} className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 overflow-hidden">
                <div className="px-4 py-2.5 bg-gray-50 dark:bg-graphite-2 border-b border-gray-200 dark:border-white/10 flex items-center justify-between">
                  <div>
                    <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">{c.customer_name}</span>
                    {c.customer_tax_id && <span className="text-xs text-gray-400 dark:text-gray-500 ml-2">NIT/CC {c.customer_tax_id}</span>}
                  </div>
                  <button
                    onClick={async () => {
                      try {
                    ...
                    className="text-xs px-2.5 py-1 rounded-md border border-gray-300 dark:border-white/10 hover:bg-gray-100 hover:dark:bg-white/5"
                  >
                ...
                <table className="min-w-full divide-y divide-gray-100 dark:divide-white/10">
                  <tbody className="divide-y divide-gray-50">
                    {c.sales.map((s) => (
                      <tr key={s.id}>
                        <td className="px-4 py-1.5 text-xs text-gray-400 dark:text-gray-500 w-24">{s.sale_date}</td>
                        <td className="px-2 py-1.5 text-sm text-gray-700 dark:text-gray-300 font-mono">{s.sale_number}</td>
                  ...
                  <tfoot>
                    <tr className="bg-gray-50 dark:bg-graphite-2/60 font-medium">
                      <td colSpan={2} className="px-4 py-1.5 text-xs text-gray-400 dark:text-gray-500 text-right">Subtotal</td>
        ...
        {!loading && data && tab === 'libro-diario' && (
          <div className="space-y-4">
            ...
            {data.entries.map((entry) => (
              <div key={entry.id} className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 overflow-hidden">
                <div className="px-4 py-2.5 bg-gray-50 dark:bg-graphite-2 border-b border-gray-200 dark:border-white/10 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
                  <span className="font-mono font-semibold text-gray-700 dark:text-gray-300">{entry.entry_number}</span>
                  <span className="text-gray-500 dark:text-gray-500">{entry.entry_date}</span>
                  ...
                  <span className="text-gray-600 dark:text-gray-400 dark:dark:text-gray-500 flex-1">{entry.description || '—'}</span>
                </div>
                <table className="min-w-full divide-y divide-gray-100 dark:divide-white/10">
                  <tbody className="divide-y divide-gray-50">
                    {entry.lines.map((l) => (
                      <tr key={l.id}>
                        <td className="px-4 py-1.5 text-xs font-mono text-gray-400 dark:text-gray-500 w-16">{l.account_code}</td>
                        <td className="px-2 py-1.5 text-sm text-gray-800 dark:text-gray-200">{l.account_name}</td>
                        <td className="px-2 py-1.5 text-xs text-gray-400 dark:text-gray-500">{l.description || ''}</td>
                  ...
                  <tfoot>
                    <tr className="bg-gray-50 dark:bg-graphite-2/60">
                      <td colSpan={3} className="px-4 py-1.5 text-xs text-gray-400 dark:text-gray-500 text-right">Subtotal</td>
            ...
            {data.entries.length > 0 && (
              <div className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 p-4 flex justify-end gap-8 text-sm font-semibold">
        ...
        {!loading && data && tab === 'libro-iva' && (
          <div className="space-y-4">
            <div className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 overflow-hidden">
              <div className="px-4 py-2.5 bg-gray-50 dark:bg-graphite-2 border-b border-gray-200 dark:border-white/10 text-sm font-semibold text-gray-700 dark:text-gray-300">IVA Generado (Ventas)</div>
              <table className="min-w-full divide-y divide-gray-200 dark:divide-white/10">
                <thead className="bg-gray-50 dark:bg-graphite-2">
                  <tr>{['Fecha', 'N° Asiento', 'Origen', 'Detalle', 'Valor'].map((h) => <th key={h} className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-500 uppercase">{h}</th>)}</tr>
                </thead>
                <tbody className="divide-y divide-gray-100 dark:divide-white/10">
                  {data.generado.length === 0 && <tr><td colSpan={5} className="px-4 py-6 text-center text-gray-400 dark:text-gray-500">Sin movimientos en el periodo</td></tr>}
                  {data.generado.map((r) => (
                    <tr key={r.entry_id}>
                      <td className="px-4 py-2 text-sm text-gray-500 dark:text-gray-500">{r.entry_date}</td>
                      <td className="px-4 py-2 text-sm font-mono text-gray-500 dark:text-gray-500">{r.entry_number}</td>
                      ...
                      <td className="px-4 py-2 text-sm text-gray-800 dark:text-gray-200">{r.description || '—'}</td>
            ...
            <div className="bg-white dark:bg-graphite rounded-xl border border-gray-200 dark:border-white/10 overflow-hidden">
              <div className="px-4 py-2.5 bg-gray-50 dark:bg-graphite-2 border-b border-gray-200 dark:border-white/10 text-sm font-semibold text-gray-700 dark:text-gray-300">IVA Descontable (Compras)</div>
              <table className="min-w-full divide-y divide-gray-200 dark:divide-white/10">
                <thead className="bg-gray-50 dark:bg-graphite-2">
                  <tr>{['Fecha', 'N° Asiento', 'Origen', 'Detalle', 'Valor'].map((h) => <th key={h} className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-500 uppercase">{h}</th>)}</tr>
                </thead>
                <tbody className="divide-y divide-gray-100 dark:divide-white/10">
                  {data.descontable.length === 0 && <tr><td colSpan={5} className="px-4 py-6 text-center text-gray-400 dark:text-gray-500">Sin movimientos en el periodo</td></tr>}
