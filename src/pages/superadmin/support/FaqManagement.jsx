    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Gestión de FAQ</h1>
            <p className="text-sm text-gray-500">Administra categorías y artículos de ayuda</p>
          </div>
        </div>
        <button
      ...
      {!faqLoading && faqCategories.length === 0 && (
        <div className="text-center py-20 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
          ...
          <p className="text-gray-500 font-medium">No hay categorías de FAQ</p>
          <p className="text-gray-400 text-sm mt-1">Crea la primera categoría para comenzar</p>
        </div>
      )}
            ...
            <div key={cat.id} className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              ...
              {isOpen && (
                <div className="border-t border-gray-100">
                  {articles.length === 0 && (
                    <p className="text-center text-gray-400 text-sm py-6">Sin artículos en esta categoría</p>
                  )}
                  {articles.map((art) => (
                    <div
                      key={art.id}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-800 truncate">{art.question}</p>
                        <p className="text-xs text-gray-400 mt-0.5 line-clamp-2">{art.answer}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setCatModal({ open: false, editing: null, form: { ...emptyCategory } })}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b">
              ...
              <button onClick={() => setCatModal({ open: false, editing: null, form: { ...emptyCategory } })} className="text-gray-400 hover:text-gray-600">
                <X className="w-5 h-5" />
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setArtModal({ open: false, editing: null, categoryId: null, form: { ...emptyArticle } })}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b">
              ...
              <button onClick={() => setArtModal({ open: false, editing: null, categoryId: null, form: { ...emptyArticle } })} className="text-gray-400 hover:text-gray-600">
                <X className="w-5 h-5" />
              </button>
            </div>
