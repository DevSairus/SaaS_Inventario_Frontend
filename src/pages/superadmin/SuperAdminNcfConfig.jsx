  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Facturación Núcleo (NCF)</h1>
      ...
      <Card>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">URL base del Núcleo</label>
            <input
              type="text"
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent dark:bg-graphite-2 dark:border-white/10 dark:text-gray-100 dark:placeholder-gray-600"
            />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">
              API Key {meta.has_api_key && <span className="text-emerald-600 text-xs dark:text-emerald-400">(configurada -- deja en blanco para no cambiarla)</span>}
            </label>
            <div className="relative">
              <input
                ...
                className="w-full px-3 py-2 pr-10 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent dark:bg-graphite-2 dark:border-white/10 dark:text-gray-100 dark:placeholder-gray-600"
              />
              <button type="button" onClick={() => setShowApiKey(!showApiKey)} className="absolute right-3 top-2.5 text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">
              Webhook secret {meta.has_webhook_secret && <span className="text-emerald-600 text-xs dark:text-emerald-400">(configurado -- deja en blanco para no cambiarlo)</span>}
            </label>
            <input
              type="password"
          ...
          {meta.webhookUrl && (
            <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg text-sm dark:bg-graphite-2 dark:border-white/10">
              <p className="font-medium text-gray-700 mb-1 dark:text-gray-300">Webhook URL a registrar en el Núcleo:</p>
          ...
          <label className="flex items-center gap-2">
            ...
            />
            <span className="text-sm text-gray-700 dark:text-gray-300">Conexión activa (si está apagado, no se envía nada al Núcleo aunque haya tenants configurados)</span>
          </label>
      ...
      <Card>
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            <label className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-500">
              <input
                type="checkbox"
                checked={forzarSync}
        ...
        {!config.is_active && (
          <div className="flex items-center gap-2 p-3 mb-4 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800 dark:bg-amber-900/30 dark:border-amber-800/40 dark:text-amber-300">
            <AlertCircle className="w-4 h-4" /> Activa la conexión arriba antes de poder sincronizar.
          </div>
        )}

        {syncResults && (
          <div className="mb-4 p-3 bg-gray-50 border border-gray-200 rounded-lg dark:bg-graphite-2 dark:border-white/10">
            <p className="text-sm font-medium text-gray-700 mb-2 dark:text-gray-300">Resultado de la última sincronización:</p>
            <div className="space-y-1 text-sm">
              {syncResults.map((r) => (
                <div key={r.tenant_id} className="flex items-center gap-2">
                  {r.ok === true && <CheckCircle className="w-4 h-4 text-emerald-600 flex-shrink-0 dark:text-emerald-400" />}
                  {r.ok === false && <AlertCircle className="w-4 h-4 text-red-600 flex-shrink-0 dark:text-red-400" />}
                  {r.ok === null && <Info className="w-4 h-4 text-gray-400 flex-shrink-0 dark:text-gray-500" />}
                  <span className="text-gray-800 dark:text-gray-200">{r.tenant}</span>
                  <span className="text-gray-400 dark:text-gray-500">--</span>
                  <span className="text-gray-500 dark:text-gray-500">{r.message}</span>
                </div>
              ))}
            </div>
          </div>
                ...
                <tr key={t.id}>
                  ...
                  <td className="py-2 pr-4">
                    {t.ncf_last_status ? (
                      ...
                      <span className="text-gray-400 text-xs dark:text-gray-500">Sin sincronizar</span>
                    )}
                  </td>
                  <td className="py-2 pr-4 text-gray-500 text-xs dark:text-gray-500">
                    {t.ncf_last_error || (t.ncf_payment_link_url && (
                      <a href={t.ncf_payment_link_url} target="_blank" rel="noreferrer" className="text-emerald-600 underline dark:text-emerald-400">
                        Ver link de pago
                      </a>
                    ))}
                  </td>
