...
// branches.routes.js: crear/editar/desactivar sede y asignar/quitar usuarios
// son admin/super_admin únicamente; listar usuarios de una sede también
// admite manager. El resto de roles solo puede ver el listado de sedes.
const CAN_MANAGE_BRANCHES = ['admin', 'super_admin'];
const CAN_VIEW_BRANCH_USERS = ['admin', 'super_admin', 'manager'];

const BranchesPage = () => {
  const { user } = useAuthStore();
  const canManageBranches = CAN_MANAGE_BRANCHES.includes(user?.role);
  const canViewBranchUsers = CAN_VIEW_BRANCH_USERS.includes(user?.role);
                    ...
                    <tr key={b.id} className="hover:bg-gray-50 transition-colors dark:hover:bg-white/5">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          {canManageBranches && b.is_active && !b.is_main && (
                            <button
