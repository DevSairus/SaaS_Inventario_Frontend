// frontend/src/components/accounting/LibroMayorModal.jsx
...
/**
 * Modal de Libro Mayor — se dispara desde el Plan de Cuentas (clic en una
 * cuenta de detalle). Muestra saldo inicial + movimientos con saldo corrido
 * y permite exportar a Excel/PDF con los mismos filtros.
 */
const LibroMayorModal = ({ account, onClose }) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await financialReportsAPI.libroMayor(account.id, { from: range.from, to: range.to, branch_id });
      if (requestIdRef.current === requestId) setData(res.data);
    } catch (error) {
      if (requestIdRef.current === requestId) toast.error(error.response?.data?.message || 'Error generando el libro mayor');
    } finally {
  ...
  const handleExport = async (format) => {
    ...
    try {
      ...
      const response = await financialReportsAPI.exportLibroMayor(account.id, { from: range.from, to: range.to, branch_id }, format);
      const url = URL.createObjectURL(response.data);
      const ext = format === 'pdf' ? 'pdf' : 'xlsx';
      const filename = `Libro-Mayor-${account.code}-${range.from}_${range.to}.${ext}`;
      ...
      toast.error(`Error generando el ${format === 'pdf' ? 'PDF' : 'Excel'} del libro mayor`);
    } finally {
      setDownloading(null);
    }
  };
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-graphite rounded-xl shadow-xl w-full max-w-3xl max-h-[85vh] flex flex-col">
        <div className="px-5 py-4 border-b border-gray-200 flex items-center justify-between dark:border-white/10">
          <div>
            <h3 className="font-semibold text-gray-900 dark:text-gray-100">Libro Mayor</h3>
            <p className="text-sm text-gray-500 font-mono dark:text-gray-500">{account.code} — {account.name}</p>
          </div>
            ...
            <div className="p-5">
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-white/10">
                <thead className="bg-gray-50 dark:bg-graphite-2">
                  <tr>
                    {['Fecha', 'N° Asiento', 'Detalle', 'Débito', 'Crédito', 'Saldo'].map((h) => (
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-white/10">
                  {data.movements.length === 0 && (
                    <tr><td colSpan={6} className="px-3 py-8 text-center text-gray-400 dark:text-gray-500">Sin movimientos en el periodo seleccionado</td></tr>
                  )}
                ...
                {data.movements.length > 0 && (
                  <tfoot>
                    <tr className="font-semibold border-t border-gray-200 bg-gray-50 dark:border-white/10 dark:bg-graphite-2">
                      <td colSpan={3} className="px-3 py-2 text-sm dark:text-gray-200">Total periodo / Saldo final</td>
