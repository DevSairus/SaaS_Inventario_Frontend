...
const I = {
  ...
  expand:    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="w-[15px] h-[15px] shrink-0"><path d="M13 5l7 7-7 7"/><path d="M5 5l7 7-7 7"/></svg>,
};
...
export default function Sidebar({ isCollapsed, setIsCollapsed, isMobileOpen, setIsMobileOpen }) {
  ...
  // Determine which group is active to open by default
  ...
  const toggleGroup = (id) => {
    if (!expanded) {
      // If sidebar is collapsed, expand it first then open the group
      ...
      {NAV.map((item) => {
        ...
        return (
          <div key={item.id}>
            ...
            {(expanded || inMobile) && (
              <div style={{
                maxHeight: isOpen ? `${item.children.length * 28}px` : "0px",
                overflow: "hidden",
  ...
  return (
    <>
        ...
        <div className="flex items-center h-10 border-b border-white/[0.06] shrink-0 px-[14px]">
          ...
          {!expanded && (
            <button
              ...
              title="Expandir menú"
              className="absolute right-0 top-0 h-10 w-full flex items-center justify-center text-gray-500 hover:text-gray-300 hover:bg-white/[0.06] transition-colors"
            >
              {/* show expand hint on hover via title only when collapsed */}
            </button>
          )}
        </div>
        ...
        <div className="border-t border-white/[0.06] shrink-0">
          ...
          {expanded ? (
            ...
            <button
              ...
              className="w-full flex items-center justify-center h-8 text-gray-500 hover:text-gray-300 hover:bg-white/5 transition-colors"
            >
              {dark
          ...
          {expanded ? (
            <div className="flex items-center gap-2.5 h-10 px-[14px]">
              ...
              <button onClick={handleLogout} title="Cerrar sesión"
                className="p-1 rounded-md text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-colors shrink-0">
                {I.logout}
              </button>
            </div>
          ) : (
            <button onClick={handleLogout} title="Cerrar sesión"
              className="w-full flex items-center justify-center h-10 text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-colors">
              {I.logout}
            </button>
          )}
        </div>
      ...
      {/* Espacio reservado para el layout desktop */}
